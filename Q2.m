...
for i = 1:length(calculatedConcentration) - 1
    ...
    if i > 5 && i < 12
        % add step loading function
        calculatedConcentration(i+1) = calculatedConcentration(i+1) + (stepLoad/(eigenValue * lakeVolume)) * (1 - exp(-eigenValue * (i)));
    end
end
x = 1994:1:2010;
